use casperlabs_types::{U512, Key, CLTyped, bytesrepr::FromBytes};
use casperlabs_engine_test_support::{Code, Hash, SessionBuilder, TestContext, TestContextBuilder, PublicKey};
...
impl ERC20Contract {
    pub fn deployed() -> Self {
        ...
        Self { context, token_hash, proxy_hash }
    }

    pub fn transfer(&mut self, reciever: PublicKey, amount: u64, sender: Sender) {
        let token_hash = self.token_hash.clone();
        self.call_proxy(sender, ((method::TRANSFER, token_hash), reciever, U512::from(amount)))
    }

    pub fn approve(&mut self, spender: PublicKey, amount: u64, sender: Sender) {
        let token_hash = self.token_hash.clone();
        self.call_proxy(sender, ((method::APPROVE, token_hash), spender, U512::from(amount)))
    }

    pub fn transfer_from(&mut self, owner: PublicKey, reciever: PublicKey, amount: u64, sender: Sender) {
        let token_hash = self.token_hash.clone();
        self.call_proxy(sender, ((method::TRANSFER_FROM, token_hash), owner, reciever, U512::from(amount)))
    }

    pub fn balance_of(&self, account: PublicKey) -> u64 {
        let balance: Option<U512> = self.query_contract(account.to_string());
        balance.unwrap_or(U512::zero()).as_u64()
    }
    ...
    pub fn allowance(&self, owner: PublicKey, spender: PublicKey) -> u64 {
        ...
        allowance.unwrap_or(U512::zero()).as_u64()
    }
