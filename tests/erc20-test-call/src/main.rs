...
use casper_erc20::{
    constants::{ARG_AMOUNT, ARG_RECIPIENT, METHOD_APPROVE, METHOD_TRANSFER, METHOD_TRANSFER_FROM},
    Address,
};
use casper_types::{
...
const APPROVE_AS_STORED_CONTRACT_ENTRYPOINT: &str = "approve_as_stored_contract";
const TRANSFER_FROM_AS_STORED_CONTRACT_ENTRYPOINT: &str = "transfer_from_as_stored_contract";
...
extern "C" fn transfer_from_as_stored_contract() {
    ...
    runtime::call_contract::<()>(token_contract, METHOD_TRANSFER_FROM, transfer_from_args);
}
...
extern "C" fn approve_as_stored_contract() {
    ...
    runtime::call_contract::<()>(token_contract, METHOD_APPROVE, approve_args);
}
...
pub extern "C" fn call() {
    ...
    let approve_as_stored_contract_entrypoint = EntryPoint::new(
        String::from(APPROVE_AS_STORED_CONTRACT_ENTRYPOINT),
        vec![
            Parameter::new(ARG_TOKEN_CONTRACT, ContractHash::cl_type()),
            Parameter::new(ARG_SPENDER, Address::cl_type()),
            Parameter::new(ARG_AMOUNT, U256::cl_type()),
    ...
    let transfer_from_as_stored_contract_entrypoint = EntryPoint::new(
        String::from(TRANSFER_FROM_AS_STORED_CONTRACT_ENTRYPOINT),
        vec![
            Parameter::new(ARG_TOKEN_CONTRACT, ContractHash::cl_type()),
            Parameter::new(ARG_OWNER, Address::cl_type()),
            Parameter::new(ARG_RECIPIENT, Address::cl_type()),
