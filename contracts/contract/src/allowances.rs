...
use crate::{
    ...
    utils::{
        get_dictionary_value_from_key, make_dictionary_item_key_value,
        set_dictionary_value_for_key, to_account_or_pachage_hash_key,
    },
};
...
pub fn write_allowance_to(owner: Key, spender: Key, amount: U256) {
    let owner = to_account_or_pachage_hash_key(owner);
    let spender = to_account_or_pachage_hash_key(spender);
