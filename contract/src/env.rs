...
use casperlabs_types::{ContractRef, Key, U512, URef, bytesrepr::{FromBytes, ToBytes}, CLTyped};
...
pub fn key<T: FromBytes + CLTyped>(name: &str) -> Option<T> {
    match runtime::get_key(name) {
        None => None,
        Some(maybe_key) => {
            let key = maybe_key.try_into().unwrap_or_revert_with(Error::UnexpectedType);
